...
export async function listMetrics(
	...
	try {
		...
		const metrics = await cacheGet(cacheKey, async () => {
			const rows = await repository.findByUserAndType(userId, type, limit, offset);
			return rows.map((row) => ({
				id: row.id,
				date: toISO(row.date),
				value: targetUnit ? fromBase(Number(row.value), targetUnit) : Number(row.value),
				unit,
...
export async function getChartData(
	...
	try {
		...
		const data = await cacheGet(cacheKey, async () => {
			...
			const rows = await repository.findChartData(userId, type, startDate, endDate);
			return rows.map((row) => ({
